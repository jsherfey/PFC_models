function [T,FS_V,FS_DS00iNa_m,FS_DS00iNa_h,FS_DS00iDR_n,FS_DS00KDyn_ko,FS_TW03iH_m]=solve_ode
% ------------------------------------------------------------
% Parameters:
% ------------------------------------------------------------
...
FS_TW03iH_m = zeros(nsamp,p.FS_Npop);
  FS_TW03iH_m(1,:) =  p.FS_TW03iH_IC+p.FS_TW03iH_IC_noise.*rand(1,p.FS_Npop);
% ###########################################################
...
for k=2:ntime
  t=T(k-1);
  FS_V_k1=(((-((p.FS_DS00iNa_gnaf.*FS_DS00iNa_m(n-1).^3.*FS_DS00iNa_h(n-1).*(FS_V(n-1)-p.FS_DS00iNa_ENa))))+((-((p.FS_DS00iDR_gkdr.*FS_DS00iDR_n(n-1).^4.*(FS_V(n-1)-((25*log(FS_DS00KDyn_ko(n-1)/p.FS_DS00iDR_ki)))))))+((-(( p.FS_TW03iH_gH.*FS_TW03iH_m(n-1).*(FS_V(n-1)-p.FS_TW03iH_E_AR))))+((-((p.FS_pas_gpas.*(FS_V(n-1)-p.FS_pas_epas))))))))+p.FS_Iapp*(t>50&t<150))./p.FS_Cm;
  ...
  FS_TW03iH_m_k1= (( p.FS_TW03iH_c_ARaM.*((( 1 ./ (1+exp((p.FS_TW03iH_AR_V12-FS_V(n-1))/p.FS_TW03iH_AR_k)))) ./ (( 1./(p.FS_TW03iH_AR_L.*exp(-14.6-.086*FS_V(n-1))+p.FS_TW03iH_AR_R.*exp(-1.87+.07*FS_V(n-1)))))))).*(1-FS_TW03iH_m(n-1))-(( p.FS_TW03iH_c_ARbM.*((1-(( 1 ./ (1+exp((p.FS_TW03iH_AR_V12-FS_V(n-1))/p.FS_TW03iH_AR_k)))))./(( 1./(p.FS_TW03iH_AR_L.*exp(-14.6-.086*FS_V(n-1))+p.FS_TW03iH_AR_R.*exp(-1.87+.07*FS_V(n-1)))))))).*FS_TW03iH_m(n-1);
  % ------------------------------------------------------------
  ...
  FS_TW03iH_m(n)=FS_TW03iH_m(n-1)+dt*FS_TW03iH_m_k1;
  n=n+1;
end
